...
#define MAX_SIZE 10000                  /* Max number to be sent */
...
static doca_error_t a_callback(void *param, void *config)
{
	struct sc_config *app_cfg = (struct sc_config *)config;
	int a = *(int *)param;
...
static doca_error_t b_callback(void *param, void *config)
{
	struct sc_config *app_cfg = (struct sc_config *)config;
	int b = *(int *)param;
...
// Declaration of the my_add function
int my_add(const char *recv_buffer) {
    int a, b;
    if (recv_buffer == NULL || strlen(recv_buffer) == 0) {
        printf("Error: Buffer is empty or NULL.\n");
        return 0;  // Returning 0 or an appropriate error code
    }

    // Parsing integers from the buffer
    if (sscanf(recv_buffer, "%d,%d", &a, &b) == 2) {
        return a + b;  // If both integers are parsed successfully, return their sum
    } else {
        printf("Error: Input does not contain two integers.\n");
        return 0;  // Returning 0 or an appropriate error code in case of error
    }
}
 ...
 */
static void *sendto_channel(void *context)
{
	struct cc_ctx *ctx = (struct cc_ctx *)context;
	struct epoll_event events[MAX_EVENTS];
	struct epoll_event send_event = {.events = EPOLLIN, .data.fd = ctx->cc_send_fd};
	...
	//sprintf(send_buffer, "Hello from Host. This is: %d", sent_count);
	//strcat(send_buffer, "my message");
	...
	while (msg_nb) {
		...
		sprintf(send_buffer, "%d,%d", ctx->cfg->a,ctx->cfg->b);

		result = doca_comm_channel_ep_sendto(ctx->ep,
						     send_buffer,
						     ctx->cfg->send_msg_size,
						     DOCA_CC_MSG_FLAG_NONE,
						     ctx->peer);
		pthread_res = pthread_mutex_unlock(ctx->mutex);
		DOCA_LOG_INFO("send message is: %s", send_buffer);
 ...
 * @return: NULL (dummy return because of pthread requirement)
 */
static void *recvfrom_channel(void *context)
{
	struct cc_ctx *ctx = (struct cc_ctx *)context;
	struct doca_comm_channel_addr_t *curr_peer;
	struct epoll_event events[MAX_EVENTS];
	struct epoll_event recv_event = {.events = EPOLLIN, .data.fd = ctx->cc_recv_fd};
	...
	while (1) {
		pthread_res = pthread_mutex_lock(ctx->mutex);
		DOCA_LOG_INFO("Received message is: %s",recv_buffer);
		if (recv_buffer[0]!=0){
			int answer = my_add(recv_buffer);
			DOCA_LOG_INFO("after calculate, we can get the answer is: %d",answer);
		}
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t init_thread_sync(pthread_mutex_t *mutex)
{
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t set_cc_properties(enum sc_mode mode,
				      struct cc_ctx *ctx,
				      struct doca_dev *dev,
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t init_cc(struct sc_config *cfg,
			    struct cc_ctx *ctx,
			    struct doca_dev **dev,
			    struct doca_dev_rep **dev_rep)
{
	...
	if (cfg->mode == SC_MODE_HOST) {
		...
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Couldn't establish a connection with DPU node: %s", doca_error_get_descr(result));
			doca_comm_channel_ep_destroy(ctx->ep);
			doca_dev_close(*dev);
			return result;
		}

		while ((result = doca_comm_channel_peer_addr_update_info(ctx->peer)) ==
 ...
 * @dev_rep [in]: Comm Channel DOCA device representor
 */
static void destroy_cc(struct cc_ctx *ctx, struct doca_dev *dev, struct doca_dev_rep *dev_rep)
{
	doca_error_t result;
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t init_signaling_polling(int *cc_send_epoll_fd,
					   int *cc_recv_epoll_fd,
					   int *send_interrupt_fd,
					   int *recv_interrupt_fd)
{
 ...
 * @recv_interrupt_fd [out]: File descriptor to catch interrupts on receive thread
 */
static void close_fd(int cc_send_epoll_fd, int cc_recv_epoll_fd, int send_interrupt_fd, int recv_interrupt_fd)
{
	close(recv_interrupt_fd);
	close(send_interrupt_fd);
	close(cc_recv_epoll_fd);
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t start_threads(struct cc_ctx *ctx)
{
...
doca_error_t register_secure_channel_params(void)
{
	...
	struct doca_argp_param *message_size_param, *messages_number_param, *pci_addr_param, *rep_pci_addr_param,*a,*b;
	...
	result = doca_argp_param_create(&a);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create ARGP param: %s", doca_error_get_descr(result));
		return result;
	}
	doca_argp_param_set_short_name(a, "a");
	doca_argp_param_set_long_name(a, "augend");
	doca_argp_param_set_description(a, "the first number that will be send to DPU");
	doca_argp_param_set_callback(a, a_callback);
	doca_argp_param_set_type(a, DOCA_ARGP_TYPE_INT);
	doca_argp_param_set_mandatory(a);
	result = doca_argp_register_param(a);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to register program param: %s", doca_error_get_descr(result));
		return result;
	}

	/* Create and register b */
	result = doca_argp_param_create(&b);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create ARGP param: %s", doca_error_get_descr(result));
		return result;
	}
	doca_argp_param_set_short_name(b, "b");
	doca_argp_param_set_long_name(b, "addend");
	doca_argp_param_set_description(b, "the second number that will be send to DPU");
	doca_argp_param_set_callback(b, b_callback);
	doca_argp_param_set_type(b, DOCA_ARGP_TYPE_INT);
	doca_argp_param_set_mandatory(b);
	result = doca_argp_register_param(b);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to register program param: %s", doca_error_get_descr(result));
		return result;
	}
